...
def check_output_quality(output: str, test_case: dict) -> dict:
    ...
    words = output.split()
    if len(words) > 5:
        # Check for repeated sequences (more than 2 times = actual loop)
        # Ignore code patterns, docstrings, and structured formats
        for window_size in [5, 10, 15]:  # Larger windows = catch real loops
            if len(words) >= window_size * 4:  # Need at least 4 occurrences
                sequence_counts = {}
                for i in range(len(words) - window_size + 1):
                    seq = " ".join(words[i:i+window_size])
                    
                    # Skip if sequence contains code markers (likely legitimate)
                    code_markers = ['def', 'return', 'import', 'class', 'elif', 'if', 'for', 'while']
                    if any(marker in seq.lower() for marker in code_markers):
                        continue
                    
                    # Skip if sequence is part of docstring format
                    doc_markers = ['args:', 'returns:', 'parameters:', 'optional):', 'step 1:', 'step 2:']
                    if any(marker in seq.lower() for marker in doc_markers):
                        continue
                    
                    sequence_counts[seq] = sequence_counts.get(seq, 0) + 1
                
                # Only flag if sequence appears 3+ times (real loop)
                for seq, count in sequence_counts.items():
                    if count >= 3:
                        issues.append(f"Real repetition loop detected ({count}x): '{seq[:50]}...'")
                        score -= 40
                        break
