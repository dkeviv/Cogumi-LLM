#!/usr/bin/env python3
"""
Phase 1: Generate Missing Questions for Lacking Domains

Generates questions specifically for domains that are below target:
- Common Sense: +2,500
- Instruction: +2,500
- Reading: +2,500
- Summarization: +2,500

Uses different FREE models for diversity:
...
# NEW models for diversity - Using Mistral Large 2 (128B)
FREE_MODELS = [
...
def create_prompt(domain: str, batch_size: int) -> str:
    ...
    return f"""Generate {batch_size} simple, straightforward {domain} questions specifically about: "{subtopic}"
...
async def call_api_async(session, model, prompt, semaphore):
    ...
    async with semaphore:
        payload = {
            ...
            "temperature": 1.2,  # Even higher for more diversity
        }
        headers = {
...
async def generate_for_domain(
    ...
    # Over-request by 2.5x to account for duplicates
    request_count = int(target_count * 2.5)
    ...
    # Save to checkpoint
    checkpoint_file = output_file.parent / f"checkpoint_{domain.lower().replace(' ', '_')}_missing.jsonl"
...
async def main():
    """Main execution."""
    console.print("\n[bold cyan]Phase 1: Generate Missing Questions for Lacking Domains[/bold cyan]")
    console.print("=" * 70)
    console.print("\n[cyan]Using NEW model for diversity:[/cyan]")
    console.print("  • Mistral Large 2 (128B)")
